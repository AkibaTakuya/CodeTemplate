...
public	:
	...
	{
		_Self __res(__n, __n);
		REP(i,__n) __res._data[i][i] = 1;
		return __res;
	}

	inline static _Self
	basic(const _Self& target)
	{ return basic(target._h); }
...
public	:
	...
	{ REP(i,_h) REP(j,_w) at(i,j) += other.at(i,j); return (*this); }

	_Self &
	operator *= (const _Self other)
	{
		_Self __res(_h, other._w);
		REP(i,__res._h) REP(j,__res._w) REP(k,_w)
			__res.at(i,j) += (*this).at(i,k) * other.at(k,j);
		*this = __res; return (*this);
	}
	...
	operator ^ (uInt64 __k)
	{
		_Self n = *this, __res = basic(*this);
		while (__k) { if (__k & 1) __res *= n; n *= n, __k >>= 1; }
		return __res;
	}
