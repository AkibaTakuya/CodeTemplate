...
public	:
	...
	explicit
	_ForwList_node(value_type __data)
	: _data(__data), _next() { }

	explicit
	_ForwList_node(value_type __data, _Self* __next)
	: _data(__data), _next(__next) { }
...
public	:
	...
	explicit
	_ForwList_iterator(_Node* __target)
	: _node(__target) { }
...
public	:
	...
	_Self
	operator ++ (int)
	{ _Self __res(*this); _node = _node->_next; return __res; }
...
public	: //
	...
	erase(iterator __pos) {
		if ((!__pos._node) || (!__pos._node->_next)) return false;
		_Node* __tmp = __pos._node->_next->_next;
		delete __pos._node->_next;
		__pos._node->_next = __tmp;
		return true;
	}
	...
	clear()
	{
		_Node* __node = _node->_next;
		while (__node) {
			_Node* __tmp = __node->_next;
			delete __node;
			__node = __tmp;
		}
		_node->_next = 0;
