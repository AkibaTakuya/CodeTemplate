...
public  :
	...
	size_t v; value_type w;
	_Graph_base() : v(), w() { }
	_Graph_base(size_t __v, value_type __w) : v(__v), w(__w) { }
	bool operator < (const _Self &other) const { return w < other.w; }
	bool operator > (const _Self &other) const { return w > other.w; }
	bool operator <= (const _Self &other) const { return w <= other.w; }
	bool operator >= (const _Self &other) const { return w >= other.w; }
};
...
private	: 
	...
	void _add_elem(size_t __u, size_t __v, _Tp __w) { _data[__u]->push_front(_Base(__v, __w)); }
public	:
	Graph() : _data() { }
	...
	inline const reference at (size_t __n) const { return *_data[__n]; }
	inline void add_edge(size_t __u, size_t __v, _Tp __w) { _add_elem(__u, __v, __w); }
